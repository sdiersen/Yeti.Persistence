...
public class EntryTable : Migration
{
    public override void Up()
    {
        Create.Table($"{DbTableNames.ENTRY_TABLE}")
            ...
            .WithColumn($"{DbEntryTable.ITEM_ID}").AsInt32().NotNullable().ForeignKey(DbTableNames.ITEM_TABLE, DbCommonColumns.ID)
            .WithColumn($"{DbEntryTable.CATEGORY_ID}").AsInt32().NotNullable().ForeignKey(DbTableNames.CATEGORY_TABLE, DbCommonColumns.ID)
