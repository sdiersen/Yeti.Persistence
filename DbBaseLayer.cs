...
public abstract class DbBaseLayer<T1, T2> where T2 : IBaseModel
{
    ...
    public virtual ReturnValue<T2> GetRowAsTransaction(int id, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        try
        {
            var row = connection.QueryFirstOrDefault<T2>(query, new { Id = id }, transaction);
            if (row != null)
            {
                returnValue.Success = true;
                returnValue.AddMessage("database", $"{TableName} row with Id: {id} found.");
    ...
    public virtual async Task<ReturnValue<T2>> GetRowAsTransactionAsync(int id, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        {
            var query = _getRowByIdSQL;
            var row = await connection.QueryFirstOrDefaultAsync<T2>(query, new { Id = id }, transaction);
            if (row != null)
            {
                returnValue.Success = true;
                returnValue.AddMessage("database", $"{TableName} row with Id: {id} found.");
    ...
    public virtual ReturnValue DeleteRow(int id)
    {
        ...
        using (var connection = new SqlConnection(_connectionString))
        {
            ...
            {
                connection.Open();
                int rowsAffected = connection.Execute(_deleteByIdSQL, new { Id = id });
                returnValue.Success = true;
    ...
    */
    ...
    /// If the query is not successful, ReturnValue.Success is false, ReturnValue.Messages contains errorMessage or nothing if the errorMessage parameter was default, and ReturnValue.Errors contains the exception message.
    /// </returns>
    protected async Task<ReturnValue> ExecuteQueryAsync(string sql,
                                        DynamicParameters parameters,
                                        string errorMessage = "",
                                        string successMessage = "")
    {
    ...
    //TODO: create queries for ReturnValue<T> and ReturnValue<List<T>> as with the previous two, these could be dangerous. User of these are responsible for validation and sanitization.
